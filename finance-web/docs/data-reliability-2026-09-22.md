...
授權依據：[授權交接書](authorization-data-reliability-2026-09-20.md)。本批只在本機實作、測試與提交，沒有推送、合併 main、部署、正式登入、正式 Firestore 存取或修改使用者瀏覽器資料。初始交付時未啟動人工預覽；使用者其後要求啟動隔離驗收版。
...
2026-09-24 最終人工回報：第三項月底日期與第四項提醒名稱直接查歷史均通過。信用卡設定每月 31 日時，9 月預定日期顯示 9/30；這是短月推算，原設定日仍保留 31。使用者直接建立洗牙提醒，可找到 5 月既有洗牙記帳。五項集中驗收均已回報通過；這不取代 Emulator、遠端 CI、正式雲端同步的發布檢查。

使用者已要求啟動 `preview:acceptance`，本機隔離驗收版於 `http://127.0.0.1:4186/` 提供。頁首須明確顯示「本機驗收版／僅本機」、登入停用；不要為測試在正式站刪除交易或清除瀏覽器。測試資料可用自行準備的 JSON 副本或合成資料。2026-09-24 回饋修正後，單元／語法、release、acceptance 與 18 項 browser smoke 均通過；人工五項亦已回報通過。
