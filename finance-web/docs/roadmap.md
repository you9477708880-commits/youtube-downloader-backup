...
- 衝突復原中心已於 2026-08-23 完成本機實作：覆蓋前的落敗版本改存 UID 隔離的 IndexedDB，保留上限為 10 份／30 天；提供逐筆摘要、選擇性整筆復原、手動匯出與刪除，僅在內部保存失敗時緊急下載 JSON。此批尚未推送或部署。
...
- Sync phases 3 and 4 are complete and deployed: localStorage uses one snapshot per unbound local / Firebase UID namespace; legacy `fin_v6_*` data migrates only to unbound local storage; Firestore v7 provides meta state, record-level documents, revision rules, deletion tombstones, a UID-scoped persisted mutation outbox, whole-record conflict choices, and verified v6 migration.
...
4. **匯入 / 雲端資料衝突策略**
   - 已具備 record revision、刪除 tombstone、明確覆蓋選項與裝置內衝突復原中心。
   - 不同 record 可自然共存；相同 record 仍採整筆本機／雲端選擇，不自動拼接金額、帳戶、分類或 fund link。
   - 下一步不是欄位級黑盒合併；若實際使用仍常遇到衝突，再評估可解釋的逐欄位人工比較介面與跨裝置復原歷史。
...
7. **月度回顧**
   - 雛形與 2.0 同期比較已完成本機版本；維持簡單、可追溯的月結視圖。
   - 財務導航第一版因重複既有數字且自評無法產生行動，已在人工驗收後移除，不再以「有問題可想」當成功能完成。
   - 若未來加入使用者自行填寫的下月行動，必須先定義實際回顧流程；若需保存文字，再設計資料模型、同步與刪除語意。
