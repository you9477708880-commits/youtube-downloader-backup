...
`codex/next` 已於 2026-08-29 完成候選功能整併，並依人工驗收意見進入既有功能收斂修正。候選發布範圍包含帳戶中心／信用卡、月度回顧 2.0、退休情境與護欄提領來源、整合於交易搜尋的週期間隔檢查、裝置資料清理前端流程，以及 PWA／同步衝突保護。獨立「財務導航」因重複既有總覽且自評問題沒有形成可執行結果，已從候選版移除。管理 Functions 雖已完成 v7 本機實作與 Emulator 測試，仍明確排除在本次發布範圍之外；Firestore Rules 沒有變更，也不需要部署。

完整 `npm test` 已於人工回報修正後再次通過：正式打包 81 個檔案、正式與驗收包啟動、驗收隔離、Auth／Firestore／Functions Emulators 20/20，以及 UI smoke 15/15。新增保護涵蓋財務導航與自評區已移除、搜尋週期間隔不重複交易列、對帳調整明細直接刪除，以及股票報酬 10%／20% 會產生不同配置漂移與再平衡金額。
...
`e01aa1c`；尚未發布的帳戶中心、月度回顧 2.0、退休情境與後續候選
...
退休頁的「情境比較」只比較目前設定、延後三年退休及每月提領降低 10%，維持只讀推導，不新增持久化欄位、不改同步或 Firestore 規則，也不把結果當成投資建議或保證。原先月度回顧內的獨立「財務導航」已在人工驗收後移除：它只是重複收入、支出、資產、負債與兩個無法操作的自評問題，沒有比現有總覽與月度回顧提供更多決策價值。

退休頁另完成護欄年度檢查與提領來源本機候選：交叉採用 Guyton-Klinger 年度決策規則，輸入上年度期初股票／債券／現金配置與股票、債券報酬後，先推估目前配置，再和目標配置比較並用超配資產支應提領以協助再平衡；現金報酬在這個簡化模型中固定視為 0%。股票負報酬時，試算先用現金、債券與獲准動用的預備金，其他來源不足才列出賣股；畫面也顯示 1–2 年等自訂預備金目標的金額與缺口。這些設定不保存、不建立交易、不改同步格式。

2026-08-29 額度更新前本機批次新增三項候選。人工驗收後，「生活紀錄提醒 A」已收斂為交易搜尋下方的選填週期間隔檢查：直接沿用同一個搜尋關鍵字，只補充最近一次、平均間隔與依自訂天數推算的下次日期，不再複製一份交易清單；條件仍不保存、不影響報表。「清除此裝置的帳務資料」只清目前 local／UID 的 snapshot、rollback、outbox、復原紀錄與 Firebase 離線快取，採兩段式中文確認且不刪雲端；管理 Functions 已能辨識 v6／v7、排除 tombstone 並對單一 UID 遞迴刪除，但仍維持不部署。
...
- 退休護欄與提領來源：年度提領決策、由期初配置與報酬推估的目前配置、目標配置、緊急預備金年數與再平衡來源順序（本機待複驗）。
- 待購項目預填大額準備表單；目前只預填，不會自動建立 fund、交易或事件。
- 交易搜尋週期間隔檢查：沿用搜尋關鍵字，以自行輸入的天數推導過往事件間隔與下次日期，不重複顯示交易明細；條件不保存、不同步（本機待複驗）。
...
2026-08-29 人工驗收回報修正批次：

- 移除重複既有總覽與資產負債資訊的獨立財務導航，以及沒有保存或後續用途的兩個自評問題。
- 生活週期間隔改為沿用交易搜尋關鍵字，只顯示日期摘要，不再複製第二個查詢欄位或最近交易清單。
- `balance_adjustment` 明細新增專用刪除按鈕、明確餘額重算警告與刪除後關閉流程。
- 退休來源先由期初配置與股票／債券報酬推估目前配置，再依相對目標的超配部分支應提領；10% 與 20% 股票報酬的差異已有 domain、view 與 browser smoke 保護。
- 完整 `npm test` 通過：單元／語法、正式打包 81 個檔案、正式與驗收包、隔離測試、Emulators 20/20、UI smoke 15/15；`git diff --check` 另於提交前執行。
- 本批沒有新增依賴或持久化欄位，沒有修改 Firestore Rules，也沒有讀寫正式 Firestore；不推送、不部署。
...
- 財務導航摘要已依人工驗收移除；退休情境比較已通過，仍只存在本機，不推送、不部署。
- 退休護欄的配置漂移與再平衡來源修正尚待複驗；目前只存在本機，不推送、不部署，也沒有任何自動交易能力。
...
- 只需複驗四項人工回報修正：財務導航已移除、週期間隔已併入搜尋、對帳調整可由明細安全刪除、退休來源會依報酬造成的配置漂移協助再平衡。
- 本批沒有修改持久化資料模型；若未來要做真正的背景定時提醒，需另行設計提醒規則、啟用／停用、通知權限、同步、遷移與刪除語意。
