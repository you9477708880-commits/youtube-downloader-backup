...
2026-09-24 驗收回饋：第一份未登入時匯出的 JSON 為 0 筆交易；登入後匯出的另一份含 457 筆，使用者回報匯入後數字正常。本機補上匯出／匯入筆數與 0 筆交易覆蓋前確認，測試通過。回饋修正已於本機提交 `0241669`；沒有推送或部署。
第二項「編輯後刪除不復活」已由使用者驗收通過；原列表灰色小「×」不易辨識，本機候選改成明示「刪除」的按鈕，未變更刪除語意。
第三項「信用卡最近預定繳款日包含今天」已由使用者截圖確認 24 日當天顯示 9/24；新版可設定每月 31 日，9 月推算為 9/30，使用者驗收通過。第四項原本須先搜尋才能建立提醒，已改成提醒名稱直接查歷史；使用者建立洗牙提醒並找到 5 月紀錄，驗收通過。
...
- 實作、測試、審核與集中人工驗收見 [2026-09-22 批次報告](data-reliability-2026-09-22.md)。人工預覽已於本機 `http://127.0.0.1:4186/` 啟動；2026-09-24 回饋修正後單元／語法、release、acceptance 與 18 項 browser smoke 通過，集中人工五項均已回報通過。
