...
2026-09-24 驗收回饋：第一份未登入時匯出的 JSON 為 0 筆交易；登入後匯出的另一份含 457 筆，使用者回報匯入後數字正常。本機補上匯出／匯入筆數與 0 筆交易覆蓋前確認，測試通過，尚待本機提交。詳見本批報告；其他人工項目仍在進行。
第二項「編輯後刪除不復活」已由使用者驗收通過；原列表灰色小「×」不易辨識，本機候選改成明示「刪除」的按鈕，未變更刪除語意。
第三項「信用卡最近預定繳款日包含今天」已由使用者截圖確認 24 日當天顯示 9/24；對 29～31 日原限制提出疑問，本機候選已擴充為 1～31 日且短月取月底，等待重新整理後驗收。第四項可搜尋到舊機油紀錄，亦建立提醒並顯示 9/27；但使用者認為必須先搜尋才可建立提醒不合理，故第四項未通過。本機候選已改成提醒名稱即查歷史，等待重新驗收。
第五項「再記一筆」已由使用者驗收通過：9/23 原交易與 9/24 新交易並存，金額、帳戶與備註相同。
...
- 實作、測試、審核與集中人工驗收見 [2026-09-22 批次報告](data-reliability-2026-09-22.md)。人工預覽已於本機 `http://127.0.0.1:4186/` 啟動；2026-09-24 回饋修正後單元／語法、release、acceptance 與 18 項 browser smoke 通過，第三與第四項待頁面重新整理後複驗。
