...
export function normalizeFinanceStateMoney(state) {
  const next = {
    ...
    accounts: normalizeList(state.accounts, (account) => {
      ...
      if ("statementDay" in account) account.statementDay = Math.min(31, Math.max(0, toMoneyInt(account.statementDay)));
      if ("paymentDueDay" in account) account.paymentDueDay = Math.min(31, Math.max(0, toMoneyInt(account.paymentDueDay)));
      return account;
    }),
